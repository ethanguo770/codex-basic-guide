...
test("covers every requested capability and keeps the editorial design accessible", async () => {
  ...
  assert.match(page, /Plan 先把步骤想清楚/);
  assert.match(page, /流程图让人一眼看懂/);
  ...
  assert.match(css, /\.compare-scene\{justify-content:flex-start;gap:8px;padding-top:16px;padding-bottom:16px/);
  assert.match(css, /\.compare-scene \.comparison\{flex:0 0 290px;min-height:0\}/);
