      ...
      <section className="stage" aria-live="polite">
        ...
        {scene === 2 && (
          <article className={`${lessonClass} compare-scene`}>
            ...
            <div className="statement"><h2>PLAN 负责想清楚怎么做，<br /><span>流程图（Mermaid）负责让人一眼看懂。</span></h2><p>一定要让 AI 多画图：文字 Plan 说明要做什么，流程图展示顺序、分支和异常。人先看图找遗漏，AI 同步修改，确认后再写代码。</p></div>
            <Capability
            ...
            <div className={revealClass(3, "comparison")}>
              <div className="compare-card native"><div className="compare-title"><span>AI 输出 01</span><h3>文字 Plan</h3></div><p>先把模块、顺序、限制和测试写清楚，让人确认 AI 是否真正理解了需求。</p><Command title="PLAN 案例" onCopy={copy}>{'$plan --direct "不要修改代码。先为 3D 资产上传、转换、预览、审核和发布制定开发 Plan；列出模块、顺序、限制、风险、测试和需要人确认的问题。"'}</Command><ul><li>拆清模块和先后顺序</li><li>列出限制、风险与验收标准</li><li>暂不改代码，先让人确认</li></ul><div className="fit">第一份产物：可审阅的文字 Plan</div></div>
              <div className="versus">VS</div>
              <div className="compare-card recommended"><div className="recommended-badge">本教程推荐</div><div className="compare-title"><span>AI 输出 02</span><h3>流程图（Mermaid）</h3></div><p>让 AI 把正常流程和失败分支都画出来。人看图发现遗漏，再同步修改流程图与 Plan。</p><Command title="流程图示例" onCopy={copy}>{"flowchart TD; A[上传] --> B{校验}; B -->|通过| C[转换]; B -->|失败| D[说明原因]; C --> E{审核}; E -->|退回| F[修改]; E -->|通过| G[发布]"}</Command><ul><li>正常步骤和失败分支一起画</li><li>人看图补充权限、退回等遗漏</li><li>图与 Plan 一起确认后再写代码</li></ul><div className="fit strong">关键：先看图确认，再让 AI 开工</div></div>
            </div>
            <div className={revealClass(4, "recommendation coral")}><b>让 AI 多画图</b><span>需求与代码 → AI 输出 Plan + 流程图 → 人看图 Review → AI 同步修改 → 确认后再写代码和测试。</span></div>
          </article>
        )}
