...
function Capability({ className = "", revealStep, purpose, when, input, action, human, output }: CapabilityProps) {
  const cellClass = (step: number) => `capability-cell ${revealStep >= step ? "revealed" : ""}`;
  return (
    <div className={`capability-anatomy ${className}`}>
      <div className={cellClass(1)}><small>01 · 它的功能</small><strong>{purpose}</strong></div>
      <div className={cellClass(1)}><small>02 · 什么时候用</small><span>{when}</span></div>
      <div className={cellClass(1)}><small>03 · 你要提供</small><span>{input}</span></div>
      <div className={cellClass(2)}><small>04 · AI 会执行</small><span>{action}</span></div>
      <div className={cellClass(2)}><small>05 · 人类控制点</small><span>{human}</span></div>
      <div className={cellClass(2)}><small>06 · 最后得到</small><span>{output}</span></div>
    </div>
  );
}
      ...
      <section className="stage" aria-live="polite">
        ...
        {scene === 0 && (
          <article className={`${lessonClass} cover`}>
            <div className="cover-copy">
              ...
              <div className={revealClass(1, "cover-thesis")}><b>核心作用</b><span>让理解可见，让执行可恢复，让结果有证据。</span></div>
        ...
        {scene === 6 && (
          <article className={`${lessonClass} compare-scene review-compare`}>
            ...
            <div className={revealClass(4, "review-example")}><b>本平台发现的 HIGH 问题</b><span>上传 ZIP 未防止路径穿越，并把 2GB 文件完整读入内存。</span><em>api/assets/upload.ts:71 · REQUEST CHANGES</em></div>
          </article>
        )}
        ...
        {scene === 7 && (
          <article className={`${lessonClass} debug-scene`}>
            ...
            <div className={revealClass(4, "debug-chain")}><span>稳定复现</span><i>→</i><span>比较环境差异</span><i>→</i><span>证明根因</span><i>→</i><span>最小修复</span><i>→</i><b>跨平台回归</b></div>
          </article>
        )}
