...
function PlanFlowchart({ language }: { language: Language }) {
  const label = (zh: string, en: string) => language === "zh" ? zh : en;
  return (
    <figure className="plan-flowchart">
      <figcaption><span>{label("MERMAID 渲染结果", "MERMAID RENDER")}</span><code>flowchart TD</code></figcaption>
      <svg viewBox="0 0 660 170" role="img" aria-label={label("3D 资产从上传、校验、转换、预览、审核到发布的流程图，包含校验失败和审核退回分支", "Flowchart for a 3D asset from upload and validation through conversion, preview, review, and publishing, including validation failure and review return branches")}>
        <defs>
          <marker id="plan-flow-arrow" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" /></marker>
        </defs>
        <g className="flow-lines">
          ...
          <path d="M406 81 H430" />
          <path d="M510 81 H570" />
          <path d="M470 112 V132" />
          <path className="flow-return" d="M437 144 H50 V99" />
        </g>
        <g className="flow-shapes">
          <rect x="12" y="64" width="76" height="34" rx="5" />
          <polygon className="decision" points="138,50 178,81 138,112 98,81" />
          <rect className="failure" x="105" y="8" width="66" height="26" rx="4" />
          <rect x="220" y="64" width="76" height="34" rx="5" />
          <rect x="330" y="64" width="76" height="34" rx="5" />
          <polygon className="decision" points="470,50 510,81 470,112 430,81" />
          <rect className="failure" x="437" y="132" width="66" height="26" rx="4" />
          <rect className="success" x="570" y="64" width="76" height="34" rx="5" />
        </g>
        <g className="flow-text">
          <text x="50" y="85">{label("上传", "Upload")}</text>
          <text x="138" y="85">{label("校验", "Validate")}</text>
          <text x="138" y="25">{label("说明原因", "Explain")}</text>
          <text x="258" y="85">{label("转换", "Convert")}</text>
          <text x="368" y="85">{label("预览", "Preview")}</text>
          <text x="470" y="85">{label("审核", "Review")}</text>
          <text x="470" y="150">{label("退回修改", "Revise")}</text>
          <text x="608" y="85">{label("发布", "Publish")}</text>
        </g>
        <g className="flow-branch-labels">
          <text x="194" y="73">{label("通过", "Pass")}</text>
          <text x="147" y="44">{label("失败", "Fail")}</text>
          <text x="535" y="73">{label("通过", "Pass")}</text>
          <text x="481" y="124">{label("退回", "Return")}</text>
        </g>
      </svg>
    </figure>
  );
